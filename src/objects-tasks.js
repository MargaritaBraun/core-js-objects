...
function mergeObjects(objects) {
  ...
  objects.forEach((item) => {
    const entre = Object.entries(item);
    entre.forEach((keyinValue) => {
      const key = keyinValue[0];
      if (Object.hasOwn(newObj, key)) {
        const val = newObj[key];
        newObj[key] = item[key] + val;
      } else {
        newObj[key] = item[key];
      }
    });
...
function compareObjects(obj1, obj2) {
  let result;
  const entre2 = Object.entries(obj2);
  entre2.forEach((pair) => {
    const key = pair[0];
...
function isEmptyObject(obj) {
  const pairInObj = Object.keys(obj);
  let result;
  if (pairInObj.length === 0) {
    result = true;
  } else {
    result = false;
  }
...
function makeWord(lettersObject) {
  const result = [];
  const allKeys = Object.keys(lettersObject);
  allKeys.forEach((key) => {
    const letter = key;
    lettersObject[key].forEach((poz) => {
      result[poz] = letter;
    });
  });
  return result.join('');
